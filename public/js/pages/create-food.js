$(document).ready(function() {
    ...
    function call_validate_require(target_check, message = null) {
        ...
        target_check.each(function() {
            ...
            if ($(this).val() == "" || $(this).val() == null) {
                $(this).closest("div").append("<div class='alert-error text-danger'>Vui lòng nhập " + field_name + "!</div>");
    ...
    $("input[name='food-logo']").on("change", function() {
        if ($(this).val() != "" && $(this).val() != null) {
            $(this).closest("div").find(".alert-error").remove();
        }
    });
    ...
    $("#food-category").on("change", function() {
        if ($(this).val() > 0)
            $(this).parent().find(".alert-error").remove();
    });
    ...
    $(".btn-submit").click(function(e) {
        ...
        var target_check = $(".food-logo");
        if (!call_validate_require(target_check, "hình ảnh")) {
            flag = false;
        }
        ...
        if ($("#food-category").val() == null ||  $("#food-category").val() == '') {
            flag = false;
            $(".pick-category").append("<div class='alert-error text-danger'>Vui lòng chọn Thể loại!</div>");
            focus_input_select('.pick-category');
        }
        
        if (flag) {
            $(".form-create-food").submit();
        } else {
            target_focus.focus();
            target_focus = null;
            flag = true;
